...
conn = sqlite3.connect('usersYALE.db')
cursor = conn.cursor()


def readYale(pessoa, imagem):
    dir = join(dirname(dirname(abspath(__file__))), 'datasets', 'Yale', 's')
    if(pessoa < 10):
        im = cv2.imread(join(dir + str(pessoa), "subject0"+ str(pessoa) +"."+str(imagem) + ".pgm"),-1)
    else:
        im = cv2.imread(join(dir + str(pessoa), "subject"+ str(pessoa) +"."+str(imagem) + ".pgm"),-1)
    return im
...
cascade = cv2.CascadeClassifier("sources\haarcascade-frontalface-default.xml")
cascadeEye = cv2.CascadeClassifier("sources\haarcascade_eye.xml")
width, height = 300, 350

samples = ["centerlight","noglasses","glasses","rightlight","leftlight","happy","sad","wink"]
total = [0,0]
for subject in range (1,11): #Pessoa 1 até 10
#for subject in range (11,16): #Pessoa 11 até 115
    list = [0,0] #[correto, incorreto] 

    for sample in samples: 

        image = readYale(subject, sample)
        ...
        for (x,y,w,h) in faces:
            ...
            #detectedEyes = cascadeEye.detectMultiScale(grayEyeRegion)
            detectedEyes = cascadeEye.detectMultiScale(region)

            # Coloca os quadrados nos olhos
        ...
        predictor = dlib.shape_predictor("sources copy\shape_predictor_68_face_landmarks.dat")
