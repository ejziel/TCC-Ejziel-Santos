...
def plotPoints(image, xlista, ylista, points):
    ...
    for i in range(0,68):
        cv2.circle(image, (int(xlista[i]), int(ylista[i])), 1, (0,255,255), thickness=2)
    for linha in points:
        cv2.line(image, (int(xlista[linha[0]]), int(ylista[linha[0]])), (int(xlista[linha[1]]), int(ylista[linha[1]])), (0,0,0)) 
...
#cv2.imshow('Equalização de Histograma',clahe_image)
#cv2.waitKey(0)
...
print('Distancias Reconhecimento:\n')
print(distancia)
...
#print('Distancias Banco de dados:\n')
#print(distanciasBD)
